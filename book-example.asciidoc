...
The above is an example of dockerizing Juypter while adding an opensaced kernel to be able to describe the story of how an object was created. The ipynb is an extension assigned to jupyter notebooks and the kernel for openscad was added for a raspery Pi by Sean Donnellan so as to give jypter a home there in the VSR network. Normally Jupyter is used with Python or R for example for data analysis while mixing code and documentation. Output can also be PDF. When openen in the proper Jupyter environment it is interactive.
